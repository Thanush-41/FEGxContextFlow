...
struct ContextEntry:TimelineEntry {let date:Date;let title:String;let detail:String;let status:String;var savedAt:Date?=nil;var isStale:Bool {guard let savedAt else{return false};return date.timeIntervalSince(savedAt)>=90 || savedAt.timeIntervalSince(date)>5}}
struct ContextProvider:TimelineProvider {
  ...
  func current(at date:Date=Date())->ContextEntry {
    ...
    return ContextEntry(date:date,title:s["title"] as? String ?? "Your next move",detail:s["lastAction"] as? String ?? "Start voice control in ContextFlow.",status:s["microphoneMuted"] as? Bool == true ? "MIC MUTED" : (s["status"] as? String ?? "ready").uppercased(),savedAt:savedAt)
  }
}
struct ContextWidgetView:View {
  ...
  var body:some View {
    VStack(alignment:.leading,spacing:9){HStack{Image(systemName:"waveform.circle.fill").foregroundStyle(blue);Text("ContextFlow").font(.headline);Spacer()};Label(entry.isStale ? "UPDATE PENDING" : entry.status,systemImage:entry.isStale ? "clock" : statusIcon).font(.caption2.bold()).foregroundStyle(entry.isStale ? .orange : isActive ? .green : blue);Text(entry.title).font(.subheadline.bold()).lineLimit(2);Text(entry.detail).font(.caption).foregroundStyle(.secondary).lineLimit(2);Spacer(minLength:0)
      voiceControl
...
struct ContextFlowWidget:Widget {
  ...
  }.configurationDisplayName("ContextFlow voice").description("Continue your match and your voice task.").supportedFamilies([.systemSmall,.systemMedium])}
}
