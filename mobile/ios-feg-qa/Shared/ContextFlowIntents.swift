...
@available(iOS 18.0,*)
struct StartContextFlowWidgetVoiceIntent: LiveActivityIntent, AudioRecordingIntent {
  static var title:LocalizedStringResource="Start voice control from the widget"
  static var description=IntentDescription("Start or resume ContextFlow voice control without opening the app.")
  static var openAppWhenRun=false
  @MainActor func perform() async throws -> some IntentResult & ProvidesDialog {
    #if CONTEXTFLOW_APP
    if !VoiceCoordinator.shared.isVoiceActive {
      VoiceCoordinator.shared.emit("connecting")
      do {
        try await VoiceCoordinator.shared.ensureSession()
        try await VoiceCoordinator.shared.start()
      } catch {
        VoiceCoordinator.shared.emit("offline",error:error.localizedDescription)
        throw error
      }
    }
    #endif
    return .result(dialog:"ContextFlow voice control is ready.")
  }
}
